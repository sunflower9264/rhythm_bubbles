...
这是 `Rhythm Bubbles` 的移动端 H5 即时战斗游戏：单入口依次推进寻光、记忆和旋律，第 3–5 战保持同一套单击旋律规则；五只海洋怪物每局随机且无重复排列，各自拥有独立的盘面反制机制。
...
- 记忆预览只显示一次并在短暂停留后隐藏；旋律预览必须同时显示所有带编号目标，不再逐个亮起。生命、计时、蓄力、Combo 等数值条统一使用轻量流动液体高光和周围微粒，`reducedMotion` 时静止。
...
- 战斗闭环固定为 5 战：五只怪物每局随机且无重复排列，前 4 战奖励三选一，第 5 战 Boss 通关；敌人实时攻击仅在 playing 阶段推进，预览、暂停、转场、奖励阶段冻结；每盘前三次点错扣护盾/生命并继续，第 4 次点错换盘，超时造成重击并换盘，仅生命归零失败；紫莓果冻按序化解连线、灯笼骗骗鱼切断捕获、铠潮寄居蟹击破弱点、星翼魔鬼鱼避开扫线、泡泡刺豚蓄刺停手；失败攻击统一由怪物本体撞屏，不要恢复触手缠绕或触手撞屏；不要让表现层直接修改生命、伤害、护盾或奖励。
